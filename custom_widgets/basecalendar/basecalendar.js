...
{  
  ...
  self.updateinterval = 600000 // 10 minutes.
  if ("updateinterval" in parameters)
  {
    self.updateinterval = parameters.updateinterval * 1000;
  }
  ...
  function updateCalendar(self) 
  {
    ...
    if ('undefined' !== typeof gapi && 'undefined' !== typeof gapi.auth2 &&  'undefined' !== gapi.client.calendar && gapi.auth2.getAuthInstance().isSignedIn.get()) {
      self.timerID = setInterval(updateCalendar, self.updateinterval, self)
      console.log("Update "+self.updateinterval);
  ...
  function listUpcomingEvents(self) {
    ...
    for (cal in self.calendars) {
      ...
      function(error)
      {
        console.log("Calendar '"+cal+"': "+error)
      });
    }
  }
